...
@inject('inputService', 'CrafterLP2007\LaraUi\Services\LaraUI\InputCvaService')
...
<div class="relative" x-data="{ uuid: Math.random().toString(20).substring(2, 20) }">
    ...
    <input
        ...
        @if($floating)
            {{ $attributes->twMerge(
                $inputService::new()(['floating' => ($attributes->whereStartsWith('wire:model')->first() &&
    ...
    @else
        {{ $attributes->twMerge(
            $inputService::new()(['default' => ($attributes->whereStartsWith('wire:model')->first() &&
            $errors->has($attributes->whereStartsWith('wire:model')->first()) &&
            $showValidation) ? 'error' : $variant])
        ) }}
        @endif
